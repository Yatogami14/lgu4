...
<body class="min-h-screen bg-gray-50">
        ...
        <div class="flex flex-col lg:flex-row lg:justify-between lg:items-center space-y-4 lg:space-y-0 mb-6">
            ...
            <div class="flex flex-col sm:flex-row items-start sm:items-center space-y-4 sm:space-y-0 sm:space-x-4">
                <div class="text-left sm:text-right">
                    ...
                    <p id="complianceScoreDisplay" class="text-xl sm:text-2xl font-bold text-green-600">100%</p>
                </div>
        ...
        <div class="bg-white rounded-lg shadow mb-6">
            <div class="p-6">
                <div class="flex items-center justify-between mb-2">
                    <span class="text-sm font-medium">Checklist Progress</span>
                    <span id="progressText" class="text-sm text-gray-600">0 of 0 completed</span>
                </div>
            ...
            <div class="space-y-4">
                    ...
                    <?php foreach ($current_checklist as $category => $items): ?>
                        <div class="bg-white rounded-lg shadow">
                            ...
                            <div class="p-6 space-y-6">
                                ...
                                ?>
                                <div class="bg-gray-50 rounded-lg p-4 border border-gray-200 checklist-item" data-required="<?php echo $item['required'] ? 'true' : 'false'; ?>">
                                    ...
                                    <div class="mb-4">
                                        <?php if ($item['input_type'] === 'checkbox'): ?>
                                            <div class="flex items-center space-x-2">
                                                <input type="checkbox" name="<?php echo $response_name; ?>" value="1" <?php echo !empty($saved_value) ? 'checked' : ''; ?> class="rounded border-gray-300 text-blue-600 focus:ring-blue-500 checklist-input">
                                        ...
                                        <?php elseif ($item['input_type'] === 'text'): ?>
                                            <textarea name="<?php echo $response_name; ?>" 
                                                      ...
                                                      rows="3" 
                                                      class="w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200 checklist-input"><?php echo htmlspecialchars($saved_value); ?></textarea>
                                        <?php elseif ($item['input_type'] === 'select' && isset($item['options'])): ?>
                                            <select name="<?php echo $response_name; ?>" 
                                                    class="w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200 checklist-input">
                                        ...
                                        <?php elseif ($item['input_type'] === 'number'): ?>
                                            <input type="number" name="<?php echo $response_name; ?>" value="<?php echo htmlspecialchars($saved_value); ?>" placeholder="Enter number" class="w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200 checklist-input">
                                        <?php endif; ?>
                                    </div>
                ...
                <?php else: ?>
                    <div class="bg-white rounded-lg shadow p-6 text-center text-gray-500">
                        ...
                        <p class="text-sm mt-2">Please configure a checklist for "<?php echo htmlspecialchars($inspection_data['inspection_type'] ?? 'Unknown'); ?>" in the <a href="checklist_management.php" class="text-blue-600 hover:underline">Checklist Management</a> page.</p>
                    </div>
                <?php endif; ?>
            </div>
        ...
        <div class="bg-white rounded-lg shadow mt-6">
            ...
            <div class="p-6 space-y-6">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div class="text-center">
                        <div id="summaryItemsCompleted" class="text-3xl font-bold text-blue-600">0</div>
    ...
    <script>
        // --- Existing AJAX and Modal scripts ---
        ...
        function calculateComplianceScore() {
            ...
            checklistItems.forEach(item => {
                ...
                if (isRequired && isCompliant) {
                    compliantRequiredItems++;
                }
            });

            // Score calculation based on REQUIRED items
            const score = (totalRequired > 0) ? Math.round((compliantRequiredItems / totalRequired) * 100) : 100;

            // Progress bar calculation based on ALL items that have a compliant answer
            const progressPercent = (totalItems > 0) ? Math.round((completedItems / totalItems) * 100) : 0;

            // Update score displays
            const scoreDisplay = document.getElementById('complianceScoreDisplay');
            const scoreHiddenInput = document.getElementById('compliance_score_hidden');
            const summaryScoreDisplay = document.getElementById('summaryComplianceScore');
            
            scoreDisplay.textContent = score + '%';
            summaryScoreDisplay.textContent = score + '%';
            scoreHiddenInput.value = score;

            [scoreDisplay, summaryScoreDisplay].forEach(el => {
            ...
            // Update progress bar and summary stats
            document.getElementById('progressText').textContent = `${completedItems} of ${totalItems} completed`;
            document.getElementById('progressBar').style.width = `${progressPercent}%`;
            document.getElementById('summaryItemsCompleted').textContent = completedItems;
        }

        document.addEventListener('DOMContentLoaded', function() {
            document.querySelectorAll('.checklist-input').forEach(input => {
                input.addEventListener('change', calculateComplianceScore);
                input.addEventListener('keyup', calculateComplianceScore);
            });
            calculateComplianceScore(); // Initial calculation on page load
        });
    </script>
</body>
</html>
